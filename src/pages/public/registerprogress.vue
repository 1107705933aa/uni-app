	...
	<view>
		<block>
			<view class="container">
				...
				<view class="wrapper">
					<view class="left-top-sign">QUERY</view>
					<view class="welcome">
						审核进度查询！
					</view>
					<view class="input-content">
						<view class="input-item">
							...
							<input type="number" name="phone" v-model="loginParams.phone" placeholder="请输入手机号" maxlength="11" />
						</view>
						...
						<button class="confirm-btn" :class="'bg-' + themeColor.name" :disabled="btnLoading" :loading="btnLoading" @tap="getProgress">
							进度查询
						</button>
					</view>
				</view>
				<view class="register-section">
					还没有账号?
					<text @tap="navTo('/pages/public/login?type=1')">马上注册</text>
					或者
		...
		methods: {
			...
			getProgress() {
				const cheRes = this.$mGraceChecker.check(
					this.loginParams,
					this.$mFormRule.getProgressByPassRule
				);
				if (!cheRes) {
					this.$mHelper.toast(this.$mGraceChecker.error);
					return;
				...
				this.$http.post(getProgress, data).then(res => {
					this.btnLoading = false;
					//进行判断 -->status == 0 -->注册失败，给出注册失败原因  1 --》注册成功 跳转到登录界面 2 -->审核中
					switch (res.status) {
						case 0:
							this.$mHelper.toast("很遗憾，您没有通过审核");
							if (res.mark != null && res.mark != '') {
								uni.showModal({
									title: '审核反馈',
									content: res.mark,
						...
						case 1:
							uni.showModal({
								...
								success: function(res) {
									if (res.confirm) {
										_self.navTo('/pages/public/login');
									} else if (res.cancel) {
