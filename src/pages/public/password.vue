		...
		<view class="password-type-2">
			...
			<view class="login-top" :class="'bg-' + themeColor.name">
				<view class="desc">
					...
					<text>{{type == 0?'找回密码！':'修改密码'}}</text>
				</view>
				<image class="login-pic" :src="loginPic"></image>
			</view>
			<view class="login-type-content">
				<image class="login-bg" :src="loginBg" :style="{height: type === 0 ? '74vw' : '110vw'}"></image>
				<view class="main">
					<view class="login-type-form" v-if="type === 0">
						<view class="input-item">
							<text class="iconfont icondianhua" :class="'text-' + themeColor.name"></text>
							<input class="login-type-input" type="number" name="mobile" v-model="resetPasswordParams.mobile" placeholder="请输入手机号码"
							 maxlength="11" />
						</view>
						<view class="input-item input-item-sms-code">
							...
							<view class="input-wrapper">
								<view class="rf-input-wrapper">
									<input type="number" class="login-type-input" v-model="resetPasswordParams.code" placeholder="请输入验证码"
									 maxlength="4" />
								</view>
					...
					<view class="login-type-form" v-else>
						<view class="input-item">
							<text class="iconfont iconwode" :class="'text-' + themeColor.name"></text>
							<input class="login-type-input"  type="number" v-model="resetPasswordParams.username" placeholder="请输入手机号" maxlength="11" />
						</view>
						<view class="input-item">
							<text class="iconfont iconmimaffffffpx" :class="'text-' + themeColor.name"></text>
							<input class="login-type-input" type="password" v-model="resetPasswordParams.oldpassword" placeholder="请输入原始密码"
							 maxlength="20" />
						</view>
						<view class="input-item">
							<text class="iconfont iconmimaffffffpx" :class="'text-' + themeColor.name"></text>
							<input class="login-type-input" type="password" v-model="resetPasswordParams.password" placeholder="请输入密码"
							 maxlength="20" />
						</view>
						<view class="input-item">
							<text class="iconfont iconmimaffffffpx" :class="'text-' + themeColor.name"></text>
							<input class="login-type-input" type="password" v-model="resetPasswordParams.password_repetition" placeholder="请输入确认密码"
							 maxlength="20" />
						</view>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			updatePassword() {
				const body = {
					phone: this.resetPasswordParams.username,
					password: this.resetPasswordParams.password,
					oldPassword: this.resetPasswordParams.oldpassword
				}
				let checkSendCode = this.$mGraceChecker.check(
					body,
					this.$mFormRule.updatePasswordRule
				);
				if (!checkSendCode) {
					this.$mHelper.toast(this.$mGraceChecker.error);
					return;
				...
				body.oldPassword = encrypt(this.resetPasswordParams.oldpassword);
				this.$http.post(updatePassword, body).then(res => {
					this.$mHelper.toast(res.message);
					setTimeout(() => {
						this.$http.delete(`${logout}`).then(() => {
					...
					this.resetPasswordParams.username = '';
