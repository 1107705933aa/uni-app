    ...
    <view
      ...
      v-if="
        (styleCateType === 'one_two_three_cover' ||
          styleCateType === 'one_two_three_text') &&
      ...
      <scroll-view scroll-y="true" class="right">
        <view v-for="(fItem, fIndex) in categoryList" :key="fItem.id">
          <view v-if="fIndex === showCategoryIndex" class="category">
            <view
              v-if="cateTop && cateTop.pic"
              class="banner"
              @tap="indexTopToDetailPage(cateTop)"
            >
              <image :src="cateTop && cateTop.pic" mode="aspectFill" />
            </view>
            <view class="box">
              <view
                class="second-category-text"
                @tap="navTo(`/pages/product/list?cateId=${fItem.id}`)"
              >
                {{ fItem.cateName }}
                <text class="iconfont iconyou" style="float: right;"></text>
                <view
                  class="list"
                  v-if="fItem.children && fItem.children.length > 0"
                >
                  <view
                    class="box"
                    v-for="tItem in fItem.children"
                    :key="tItem.id"
                    @tap.stop="navTo(`/pages/product/list?cateId=${tItem.id}`)"
                  >
                    <block v-if="styleCateType === 'one_two_three_cover'">
                      <image :src="tItem.pic || errorImage"></image>
                      <view class="text">{{ tItem.cateName }}</view>
                    </block>
                    <text
...
export default {
  ...
  data() {
    return {
      ...
      cateTop: {}, // 分类通用广告图
      loading: true,
      styleCateType: this.$mSettingConfig.styleCateType, //取三级分类有图
  ...
  methods: {
      ...
      await this.$http
        ...
        .then(async (r) => {
          ...
          if (cateId) {
            ...
            if (
              this.styleCateType === 'one_two_product' &&
              r.content[0].children.length > 0
            ) {
              this.currentCateId = r.content[0].children[0].id;
            } else {
          ...
          if (r.content.length > 0) {
            this.currentSecondCategoryList = r.content[0].children;
          }
          // this.page = 0;
          // this.categoryProductList = [];
    ...
    // 获取广告列表
    async getAdvList() {
      this.cateTop = []; //广告列表暂时为空
      // await this.$http
      // 	.get(`${advList}`, {
      // 		location: 'cate_top'
      // 	})
      // 	.then(r => {
      // 		this.cateTop = r.content.cate_top[0];
      // 	});
    },
    // 分类切换显示
    showCategory(index) {
