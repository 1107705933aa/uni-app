		...
		<view class="rf-header-box">
			...
			<view class="rf-header" >
				...
				<view class="input-box" >
					<!-- #endif -->
					<input v-model="keyword" @confirm="search" placeholder="请输入关键字" placeholder-style="font-size: 24upx; color:#ccc;"
					 type="text" />
		...
		<view class="rf-header-screen" >
			...
			<view class="rf-screen-bottom">
				<block v-for="(item,index) in attrArr" :key="index">
					<view class="rf-bottom-item rf-icon-ml" :class="[item.isActive ? `bg-${themeColor.name} rf-btmItem-active` : 'rf-btmItem-normal',attrIndex==index?'rf-btmItem-tap':'']"
					 :data-index="index" @tap="btnDropChange">
						...
						<text class="iconfont" :class="attrIndex==index?'iconshang':'iconxia'" :size="14" :color="attrIndex==index || item.isActive?'$base-color':'#444'"
						 rf-icon-class="rf-ml" v-if="item.list.length>0"></text>
					</view>
				</block>
			</view>
		</view>
					...
					<view class="rf-drawer-content rf-flex-attr">
						<view class="rf-attr-item" :class="[item.isActive ? `bg-${themeColor.name} rf-btmItem-active` : 'rf-btmItem-normal']"
						 v-for="(item, index) in productCateList" :key="item.id" @tap.stop="cateBtnSelected(index)">
							<view class="rf-attr-ellipsis">{{ item.cateName }}</view>
						</view>
					</view>
...
<script>
	...
	import rfProductList from '@/components/rf-product-list/index.vue';
		...
		methods: {
			...
			toggleSpec(row) {
				...
				if (this.specClass === 'show') {
					...
					if (row.type.toString() === '1') {
						...
						list.content = JSON.stringify(data);
				...
				await this.$http
					...
					.then(async () => {
						await this.$http.get(`${cartItemCount}`).then(async r => {
							this.cartNum = r.content;
							this.setCartNum(r.content);
						});
			...
			async getProductCate() {
				await this.$http
					.get(`${productCate}`)
					.then(r => {
						this.productCateList = r.content;
					});
			},
			// 获取商品品牌列表
			async getBrandList() {
