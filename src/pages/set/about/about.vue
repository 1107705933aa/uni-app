<template>
	<view class="about" v-if="!loading">
		<view class="list-cell b-b" v-for="item in navList" @tap="
				navTo(`/pages/set/about/detail?field=${item.url}&title=${item.title}&type=${item.type}`)
			" :key="item.id"
		...
		<view class="history-section icon">
			...
			<view class="qrcode-wrapper">
				<view class="qrcode-img-wrapper">
					...
					<rf-image class="qrcode-img" :src="aboutInfo.image || ''" ></rf-image>
				</view>
		...
		methods: {
			...
			getRegistration() {
				this.$http.get(getRegistration, {
					...
					if (res) {
						let data = {};
						data.title = res.content[0].registrationName;
						data.url = 'protocol_register';
						data.type = 1;
						this.navList.push(data);
						data.title = res.content[0].secretName;
						data.url = 'protocol_register';
						data.type = 2;
						this.navList.push(data);
