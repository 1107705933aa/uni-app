		...
		<view class="user-section" :class="'bg-' + themeColor.name">
			...
			<view class="user-info-box">
				<view class="portrait-box" @tap="navTo('/pages/user/userinfo/userinfo')">
					<image v-show="isAvatar" class="portrait" :src="userInfo.avatar || headImg" >
					<image v-show="!isAvatar" class="portrait" :src="loginPic" >
						<text class="username">
							{{
		 ...
		 @touchstart="coverTouchstart" @touchmove="coverTouchmove" @touchend="coverTouchend">
			...
			<view class="promotion-center">
				...
				<view class="tj-sction">
					<view class="tj-item" v-for="item in amountList" :key="item.title" @tap="jumpCart(item.url)">
			...
			<!-- 我的订单 -->
			<view class="promotion-center">
				<list-cell icon="iconicon1" :iconColor="themeColor.color" title="全部订单" @eventClick="navTo(`/pages/order/order?state=-1`)"></list-cell>
				<view class="order-section">

					<view class="order-item" open-type="switchTab" v-for="item in orderSectionList" :key="item.title" @tap="navTo(item.url)" hover-class="common-hover"
					 :hover-stay-time="50">
						<i class="iconfont" :class="[item.icon, 'text-'+themeColor.name]" />
						<text>{{ item.title }}</text>
						<rf-badge v-if="item.num > 0" type="error" size="small" class="badge" :text="item.num"></rf-badge>
					</view>
				</view>
			</view>
			<!--卡包-->
			<view class="promotion-center">
				<list-cell icon="iconfuwu" :iconColor="themeColor.color" title="卡包"></list-cell>
				<view class="tj-sction">
					<!-- 分类列表 -->
					<view class="category-list">
						<view class="category" v-for="(item, index) in settingList" :key="index" :style="{display: 'block'}" @tap.stop="navTo(item.url)">
							<view>
								<view class="img">
	...
	export default {
		...
		data() {
			return {
				...
				loginPic:this.$mAssetsPath.loginPic,//默认头像
		...
		async setCartItemCount() {
			...
			async initNotifyNum() {
				if(this.userInfo.avatar != null){
					this.userInfo.avatar = 'data:image/jpg;base64,'+ this.userInfo.avatar;
				}else {
					this.isAvatar = false;
				}
				this.$http.get(notifyUnRreadCount).then(res=>{
					this.amountList[0].value = res.cartNum || 0; //购物车
					this.amountList[1].value = res.myBookNum || 0; //我的预约
					this.amountList[2].value = res.collectionNum || 0; //收藏夹
					this.orderSectionList[0].num = res.noPayOrderNum || 0; //待付款
					this.orderSectionList[1].num = res.payOrderNum || 0;//已付款
					this.orderSectionList[2].num = res.noCheckOrderNum || 0;//待检测
					this.orderSectionList[3].num = res.allOrderNum || 0;//已完成
					this.loading = false;
				})
			},
			// 清空个人中心的各模块状态
