	...
	<view class="cart">
		...
		<view v-if="cartList.length === 0" class="empty">
			...
			<view v-if="hasLogin" class="empty-tips">
				空空如也
				<navigator class="navigator" :class="'text-'+themeColor.name" v-if="hasLogin" url="../index/index" open-type="switchTab">随便逛逛></navigator>
			</view>
		...
		<view class="goods-list" v-else>
			...
			<view class="rf-cart-row" v-for="(row, index) in cartList" :key="index">
				<!-- 删除按钮 -->
				<view class="menu" @tap.stop="deleteCartItem(row.cart_id, 'one')" :class="'bg-'+themeColor.name">
					<text class="iconfont iconnodata-search"></text>
				</view>
				<!-- 商品 -->
				 ...
				 @touchstart="touchStart(index, $event)" @touchmove="touchMove(index, $event)" @touchend="touchEnd(index, $event)">
					...
					<view class="checkbox-box" @tap="selected(index, row)">
						<view class="checkbox" :class="[
								parseInt(row.status, 10) === 1 ? `checkbox-disabled ${'text-'+themeColor.name}` : 'text-'+themeColor.name
							]">
					...
					<view class="goods-info">
						...
						<view class="info">
							<view class="title in2line" @tap="navToProduct(`/pages/trainservice/train-course-deatail?id=${row.product_id}`,row.status)">
								{{ row.product_name }}
							</view>
							<view class="state-wrapper">
								<view class="spec" @tap.stop="toggleSpec(row)">{{ row.sku_name || singleSkuText }}</view>
								<view class="state" v-if="parseInt(row.status, 10) === 1">
									已失效
								</view>
							</view>
							<view class="price-number">
								<view class="price" v-if="parseInt(row.status, 10) === 0">{{ moneySymbol }}{{
									row.price
								}}</view>
								<view class="remark" v-else>{{ row.remark }}</view>
								<view class="number" v-if="parseInt(row.status, 10) === 0">
									<text>培训课程*1</text>
		...
		methods: {
			...
			async deleteCartItem(id, type) {
				...
				if (type) {
					...
					shopcart.push(cartinfo);
				} else {
					for (let i = 0; i < this.cartList.length; i++) {
						if (this.cartList[i].selected) {
							let cartinfo = {
								id: this.cartList[i].cart_id
							};
							shopcart.push(cartinfo);
						}
					}
			...
			// 选中商品
			selected(index, item) {
				if (parseInt(item.status, 10) === 1) return;
				this.cartList[index].selected = !this.cartList[index].selected;
				let i = this.selectedList.indexOf(this.cartList[index].cart_id);
				i > -1 ?
					this.selectedList.splice(i, 1) :
					this.selectedList.push(this.cartList[index].cart_id);
			...
			allSelect() {
				...
				for (let i = 0; i < len; i++) {
					// 当商品
					if (parseInt(this.cartList[i].status, 10) !== 1) {
						this.cartList[i].selected = !this.isAllselected;
						arr.push(this.cartList[i].id);
					}
				}
				this.selectedList = this.isAllselected ? [] : arr;
				this.isAllselected = !(this.isAllselected || arr.length === 1);
				if (arr.length > 1) {
					this.sum();
				}
			},
			...
			async createOrder() {
				...
				for (let i = 0; i < len; i++) {
					if (this.cartList[i].selected) {
						data.push(this.cartList[i].product_id);
						cartids.push(this.cartList[i].cart_id); //购物车ID
					}
				}
				param.type = 'cart';
				param.data = data;
				...
				this.isAllselected = false;
				this.sumPrice = 0;
				this.navTo(`/pages/order/create/order?data=${JSON.stringify(param)}`);
			},
			// 合计
			sum() {
				this.sumPrice = 0;
