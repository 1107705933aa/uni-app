<template>
	<view>
		<view class="product-list-wrapper">
			<rf-product-list :eid="'traincourseId'" :bottom="0" :list="productList" :isList="isList" :style="{ paddingTop: dropScreenH + 10 + 'upx' }"></rf-product-list>
		</view>
		<rf-load-more :status="loadingType" v-if="productList.length > 0"></rf-load-more>
		<!--页面加载动画-->
		<rf-back-top :scrollTop="scrollTop"></rf-back-top>
		<rfLoading isFullScreen :active="loading"></rfLoading>
		<rf-empty class="empty" info="暂无课程信息" v-if="productList.length === 0 && !loading"></rf-empty>
	</view>
</template>
