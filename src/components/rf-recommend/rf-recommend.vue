<template>
	<view class="rf-floor-index">
		...
		<rf-product-list :eid="'traincourseId'" :bottom="bottom" :list="guessYouLikeList" :isList="false"></rf-product-list>
	</view>
</template>
<script>
	/**
	...
	export default {
		...
		methods: {
			...
			async getGuessYouLikeList() {
				...
				await this.$http.get(`${guessYouLikeList}`, this.params).then(r => {
