  ...
  <view id="navigatorMenu" class="navigator-menu" v-if="list && list.length">
    ...
    <view v-if="isScroll && showScrollBar" class="scroll-bar-wrap">
      <view class="scroll-groove">
        <view
          class="scroll-bar"
          :style="{ transform: 'translateX(' + barScrollLeft + ')' }"
        ></view>
      </view>
    </view>
  </view>
  ...
  methods: {
    ...
    init() {
      ...
      node.exec((result) => {
        ...
        let more_col =
          that.list.length > that.rowCount * that.colCount &&
          that.list.length % (that.rowCount * that.colCount) === 0;
        //单个宽度
        let itemWidth = `${numeral(wrap.width)
          .divide(that.colCount)
          .value()}px`;
        that.itemWidth = itemWidth;
        //外部宽度
        let wrapWidth = `${
          more_col ? itemWidth * (that.colCount + 1) : wrap.width
        }px`;
