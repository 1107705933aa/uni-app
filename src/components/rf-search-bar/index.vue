<template>
  <view class="rf-search-bar">
    ...
    >
      ...
      <view class="addr">
        <text
          class="iconfont icon-shouyeyoushangjiao_saoyisao"
          v-if="isOpenScan"
          @tap.stop="link"
          :class="[icon, 'text-' + themeColor.name]"
        ></text>
      ...
      if (this.$mPayment.isWechat()) {
        jweixin.ready(() => {
          jweixin.scanQRCode({
            ...
            success(res) {
              _this.$http
                .post(productVirtualVerificationVerify, { code: res.resultStr })
                .then(() => {
                  _this.$mHelper.toast('核销成功');
                });
            },
...
.rf-search-bar {
  ...
  .header {
    ...
    .addr {
      ...
      .icon-shouyeyoushangjiao_saoyisao {
